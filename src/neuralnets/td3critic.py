...
class TD3Critic(NeuralNet):
    ...
    def create_model(self, input_d, hidden_d, hidden_act, output_d, output_act):
        return TD3CriticNet(input_d, hidden_d, hidden_act, output_d, output_act, self.lr, self.lre, self.name, self.sess)
...
if __name__ == '__main__':
    ...
    learner = True
    name = 'tsc0'
    TD3 = TD3Critic(input_d, hidden_d, 'relu', output_d, 'tanh', lr, lre, tau, learner=learner, name=name, sess=None)
    weights = TD3.get_weights('online')
    print(type(weights))
    print(weights)
    print(weights[0])
